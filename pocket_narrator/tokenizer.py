...
# instead of the function "get_tokenizer" we can use: 
''' def get_tokenizer(
    ...
    """
    Factory for tokenizer lifecycle:
      - If tokenizer_path exists -> load
      - Else if train_corpus given -> train and (optionally) save
    """
    if tokenizer_type != "character":
        raise ValueError(f"Unknown tokenizer type: '{tokenizer_type}'")
